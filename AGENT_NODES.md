...
|---|---|
| 目标 | 把工具执行结果喂回给模型 |
| 作用 | 完成"行动→观察"的闭环；没有这一步，模型发起了调用却永远等不到结果回填 |
...
| 怎么做 | ① 危险分级注册表 `DANGEROUS_TOOLS`（工具名→危险原因）；② 循环里拦截：模型声明调用 sensitive 级工具 → `ask_human()` 终端 y/n；③ **无论批准还是拒绝，回执文本都作为 tool 结果喂回模型**——拒绝的回执写着"不要重复尝试，改为直接输出内容"，实测模型收到拒绝后尊重决定并给出替代方案 |
