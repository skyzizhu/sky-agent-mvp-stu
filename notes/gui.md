...
## 正式基线 + 首次调优闭环（2026-09-18）
- 全量12题基线（生产内核，标准档）：总分 0.68 | 有据0.65/有源0.52/覆盖0.88/简洁0.67
  637k tok / 28分钟（成本约¥0.5-1）
- 两大失败模式：①有源0.52（引用不用[n](url)格式，6题命中）②数字编造（证据中无支撑的数字）
- 两个提示词级修复：引用格式硬性化（模板+参考小节）；数字纪律（无证据标"未验证"）
- 子集复测（前4题）：0.74 → 0.81，sourced 1.00 全绿 ✓；q04 0.46→0.75
- 遗留：supported 0.66（深层原因=证据截断+懒加载，需更大改造）；q02/q03 微降属噪声区间
- 待办：全量12题复测确认；supported 深度优化（证据覆盖率）

## 修复前后全量对比（2026-09-18，正式回归）
| 维度 | 修复前 | 修复后 | Δ |
|---|---|---|---|
| 总分 | 0.68 | **0.76** | +0.08（+12%） |
| sourced 有源 | 0.52 | **0.93** | +0.41（引用格式硬性化的因果确认） |
| supported 有据 | 0.65 | 0.62 | -0.03（噪声内；数字纪律让模型更多标"未验证"） |
| complete 覆盖 | 0.88 | 0.83 | -0.05（噪声内） |
| concise 简洁 | 0.67 | 0.66 | 持平 |
- 成本不变：两次全量均 637k tok / ~28 分钟
- 结论：引用格式修复在全量规模因果确认；supported 仍是短板，
  根因=证据覆盖率（4000字符截断+懒加载内容），是下一个真正的优化项

## max_steps 出口强制收尾（2026-09-17 追加4，不限档首跑发现）
- 不限档 + 深度问题 → 20 步耗尽 → 旧代码只丢占位符"（达到最大步数…重试）"
- 修复：max_steps 出口复用硬收尾三连（trim 未应答点菜 → 无tools强制结题 → 事件发射）
  + 死线注入 == 改 >=（19/20 两步都提醒）
- 验证（MAX_LOOP_STEPS=4 运行时压测）：stop=max_steps，产出 6,047 字
  《阶段性结题报告》（含来源对照表）而非占位符 ✓
- PITFALLS #50

## 运行状态条 + 交互细节（2026-09-17 追加5）
- 顶部运行状态条：⏳ 运行中 · 第N/20步 · 档位 · 计时器（运行中显示，结束隐藏）
- 运行中禁用档位按钮（防误触改档——改了也不影响当前运行，但会让UI撒谎）
- Ctrl/⌘+Enter 快捷提交
- SSE onerror：断连时显示"连接中断"提示（此前静默关流，用户以为卡死）
